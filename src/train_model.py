...
def train_xgboost_tuned(X_train, y_train, w_train, X_val, y_val, w_val):
    ...
    X_vl = X_val.loc[:, ~X_val.columns.duplicated()]
    model = XGBRegressor(
        n_estimators=2000,            # max boosting rounds (early stopping will cut short)
        max_depth=8,                   # deeper trees to capture complex patterns
        learning_rate=0.03,            # smaller steps for finer optimization
        subsample=0.7,
        colsample_bytree=0.7,
        reg_alpha=0.1,                # L1 regularization (stronger than default)
        reg_lambda=2.0,               # L2 regularization (stronger than default)
        min_child_weight=10,
